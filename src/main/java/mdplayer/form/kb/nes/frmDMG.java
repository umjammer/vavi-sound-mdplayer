...
//    @Override
    ...
    public void screenChangeParams() {
        ...
        //pan
        newParam.channels[0].pan = (dat.sndControl.mode1Left * 2) + dat.sndControl.mode1Right;
        newParam.channels[1].pan = (dat.sndControl.mode2Left * 2) + dat.sndControl.mode2Right;
        newParam.channels[2].pan = (dat.sndControl.mode3Left * 2) + dat.sndControl.mode3Right;
        newParam.channels[3].pan = (dat.sndControl.mode4Left * 2) + dat.sndControl.mode4Right;

        //freq
        newParam.channels[0].freq = dat.snd1.frequency;
        newParam.channels[1].freq = dat.snd2.frequency;
        newParam.channels[2].freq = dat.snd3.frequency;
        newParam.channels[3].freq = dat.snd4.reg[3] & 0x7;//pfq
        newParam.channels[3].bit[47] = (dat.snd4.reg[3] & 0x8) != 0;//poly
        newParam.channels[3].srcFreq = (dat.snd4.reg[3] & 0xf0) >> 4;//pc

        //CC
        newParam.channels[0].bit[0] = dat.snd1.lengthEnabled;
        newParam.channels[1].bit[0] = dat.snd2.lengthEnabled;
        newParam.channels[2].bit[0] = dat.snd3.lengthEnabled;
        newParam.channels[3].bit[0] = dat.snd4.lengthEnabled;

        //Ini
        newParam.channels[0].bit[1] = (dat.snd1.reg[4] & 0x80) != 0;
        newParam.channels[1].bit[1] = (dat.snd2.reg[4] & 0x80) != 0;
        newParam.channels[2].bit[1] = (dat.snd3.reg[4] & 0x80) != 0;
        newParam.channels[3].bit[1] = (dat.snd4.reg[4] & 0x80) != 0;

        //Env.Dir
        newParam.channels[0].bit[2] = dat.snd1.envelopeDirection == 1;
        newParam.channels[1].bit[2] = dat.snd2.envelopeDirection == 1;
        //newParam.channels[2].bit[2] = ないよ
        newParam.channels[3].bit[2] = dat.snd4.envelopeDirection == 1;

        //Sweep Dec
        newParam.channels[0].bit[3] = dat.snd1.sweepDirection == -1;

        //Env.Spd
        newParam.channels[0].inst[0] = dat.snd1.envelopeTime;
        newParam.channels[1].inst[0] = dat.snd2.envelopeTime;
        //newParam.channels[2].inst[0] = ないよ
        newParam.channels[3].inst[0] = dat.snd4.envelopeTime;

        //Env.Vol
        newParam.channels[0].inst[1] = dat.snd1.envelopeValue;
        newParam.channels[1].inst[1] = dat.snd2.envelopeValue;
        //newParam.channels[2].inst[1] = ないよ
        newParam.channels[3].inst[1] = dat.snd4.envelopeValue;

        //Len
        newParam.channels[0].inst[2] = dat.snd1.length;
        newParam.channels[1].inst[2] = dat.snd2.length;
        //newParam.channels[2].inst[2] = ないよ
        newParam.channels[3].inst[2] = dat.snd4.length;

        //Duty
        newParam.channels[0].inst[3] = dat.snd1.duty;
        newParam.channels[1].inst[3] = dat.snd2.duty;
        //newParam.channels[2].inst[3] = ないよ
        //newParam.channels[3].inst[3] = ないよ

        //Sweep time
        newParam.channels[0].inst[4] = dat.snd1.sweepTime;
        //Sweep shift
        newParam.channels[0].inst[5] = dat.snd1.sweepShift;

        //Len
        newParam.channels[2].inst[4] = dat.snd3.length;
        //Vol
        newParam.channels[2].inst[5] = dat.snd3.level;

        //wf
        for (int i = 0; i < 16; i++) {
            newParam.wf[i * 2] = (byte) ((dat.sndRegs[0x20 + i] >> 4) & 0xf);
            newParam.wf[i * 2 + 1] = (byte) (dat.sndRegs[0x20 + i] & 0xf);
        }

        int r = 10;
        newParam.channels[0].volumeL = Math.min((dat.snd1.envelopeValue * dat.sndControl.mode1Left) * 16 / r, 19);
        newParam.channels[0].volumeR = Math.min((dat.snd1.envelopeValue * dat.sndControl.mode1Right) * 16 / r, 19);
        newParam.channels[1].volumeL = Math.min((dat.snd2.envelopeValue * dat.sndControl.mode2Left) * 16 / r, 19);
        newParam.channels[1].volumeR = Math.min((dat.snd2.envelopeValue * dat.sndControl.mode2Right) * 16 / r, 19);
        int lvl = dat.snd3.level == 0 ? 0 : (19 >> (dat.snd3.level - 1));
        newParam.channels[2].volumeL = Math.min(lvl * dat.sndControl.mode3Left * 19 / r, 19);
        newParam.channels[2].volumeR = Math.min(lvl * dat.sndControl.mode3Right * 19 / r, 19);
        newParam.channels[3].volumeL = Math.min((dat.snd4.envelopeValue * dat.sndControl.mode4Left) * 16 / r, 19);
        newParam.channels[3].volumeR = Math.min((dat.snd4.envelopeValue * dat.sndControl.mode4Right) * 16 / r, 19);
