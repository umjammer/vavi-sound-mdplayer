...
import mdplayer.chips.C352Chip.Params;
...
import static mdplayer.form.kb.pcm.FormC140.drawChC140_P;


public class FormC352 extends FormChipBase<Params> {

    static final Preferences prefs = Preferences.userNodeForPackage(FormC352.class);

    public FormC352(FormMain frm, int chipId, int zoom, C352Chip.Params newParam, C352Chip.Params oldParam) {
        super(frm, chipId, zoom, newParam, new C352Chip.Params());
    ...
    public void changeScreenParams() {
        int[] c352Register = (int[]) audio.plugin.chipRegister.chip(C352Chip.class).getInfo(chipId).get("register");
        int[] c352key = (int[]) audio.plugin.chipRegister.chip(C352Chip.class).getInfo(chipId).get("flags");
