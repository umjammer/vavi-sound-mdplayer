...
public class NRTDRV extends BaseDriver
    {
        public NRTDRV(Setting setting)
        {
            ...
            CTC1Step = 4000000.0f / (int)setting.getOutputDevice().getSampleRate();
        }
        ...
        @Override public boolean init(byte[] nrdFileData, ChipRegister chipRegister, EnmModel model, EnmChip[] useChip, int latency, int waitTime)
        {
        ...
        @Override public boolean init(byte[] vgmBuf, int fileType, ChipRegister chipRegister, EnmModel model, EnmChip[] useChip, int latency, int waitTime)
        {
            throw new UnsupportedOperationException("このdriverはこのメソッドを必要としない");
        }

        @Override public Vgm.Gd3 getGD3Info(byte[] buf, int vgmGd3)
        {
            ...
            if ((buf[2] & 0x08) != 0)
            {
                gd3.lyrics = new ArrayList<Tuple3<Integer, Integer, String>>();
                int adr = (int)vgmGd3;
                while (buf[adr] != (byte) 0xff || buf[adr + 1] != (byte) 0xff)
                {
                    int cnt = buf[adr] + buf[adr + 1] * 0x100;
                    int sAdr = (int)(buf[adr + 2] + buf[adr + 3] * 0x100);
                    String msg = Common.getNRDString(buf, sAdr);
                    gd3.lyrics.add(new Tuple3<Integer, Integer, String>(cnt, (int)sAdr, msg));
                    adr += 4;
                }
            }
        ...
        public int checkUseChip(byte[] buf)
        {
            ...
            Boolean useOPM1 = false;
            Boolean useOPM2 = false;
            Boolean usePSG = false;
            Boolean flg = false;
        ...
        private float CTCStep = 4000000.0f / 1;// setting.getoutputDevice().SampleRate;
        private float CTC1Step = 4000000.0f / 1;// (int)setting.getoutputDevice().SampleRate;

        @Override public void oneFrameProc()
        {
            try
            {
        ...
        private void pinit(int hl)
        {
            ...
            for (byte b = 0; b < 3; b++)
            {
                int de = (int)(ram[hl] + ram[hl + 1] * 0x100);
                hl += 2;
                wChs[b].ptrData = (int)(de + work.bgmadr);
                wreset(wChs[b]);
        ...
        public class Ch
        {
            ...
            public repBuf[] repBuf = new repBuf[] { new repBuf(), new repBuf(), new repBuf(), new repBuf(), new repBuf(), new repBuf() };
        }
        ...
        private void imain()
        {
            ...
            for (Ch wch : work.OPM1Chs)
            {
                wch.TrackStopFlg = (byte) 0xff;
            }

            for (Ch wch : work.OPM2Chs)
            {
                wch.TrackStopFlg = (byte) 0xff;
            }

            for (Ch wch : work.PSGChs)
            {
                wch.TrackStopFlg = (byte) 0xff;
            }
...
MAINL:
        ...
        private int VSETL(Ch wch, byte e)
        {
            ...
            wopm(d, a);
            wch.ptrData++;
            int bc = (int)(ram[wch.ptrData] + ram[wch.ptrData + 1] * 0x100);
            wch.ptrData+=2;
            int hl = (int)(work.bgmadr + bc);
        ...
        private void VSETJ(Ch wch, byte e,int hl)
        {
            byte d = (byte)(0x40 + e);
            byte a = e;
        ...
        private int GLIDE(Ch wch, byte e)
        {
            ...
            wch.PortaFlg = a;
            wch.ptrData++;
            wch.Glide = (int)(ram[wch.ptrData] + ram[wch.ptrData + 1] * 0x100);
            wch.ptrData += 2;//CCRET
            return 0;
        }
        ...
        private void volops(Ch wch,byte a,byte e)
        {
            if ((a & 1) != 0)
            {
                volwr(wch, (byte) 0x60, (byte) 0x8, e);
            }
            if ((a & 2) != 0)
            {
                volwr(wch, (byte) 0x70, (byte) 0x9, e);
            }
            if ((a & 4) != 0)
            {
                volwr(wch, (byte) 0x68, (byte) 0xa, e);
            }
            if ((a & 8) != 0)
            {
                volwr(wch, (byte) 0x78, (byte) 0xb, e);
            }
        }

        private void trkend(Ch wch)
        {
            int bc = wch.MacroReturnAdr;
            if (bc != 0)
            {
                wch.ptrData = bc;
            ...
            {
                bc = (int)(ram[wch.ptrData] + ram[wch.ptrData + 1] * 0x100);
                wch.ptrData = (int)(work.bgmadr + bc);
                wch.loopCounter++;
            }
            //ccret1は不要
        }
        ...
        private void keyon(Ch wch,byte e, byte cmdno)
        {
            ...
            {
                wch.PortaFlg = a;
                wch.PortaTone = (int)(b * 0x100 + c);
                wch.PortaStartFlg = a;

                int hl = (int)(b * 0x100 + c);
                bc = wch.Glide;
                hl += bc;
                b = (byte)(hl / 0x100);
                c = (byte)(hl & 0xff);
...
PMAINL:
        ...
        private int PVSET(Ch wch, byte e)
        {
            int bc = (int)(ram[wch.ptrData] + ram[wch.ptrData + 1] * 0x100);
            wch.ptrData += 2;
            wch.PSGToneStartAdr = (int)(work.bgmadr + bc);
        ...
        {
            wpsg((byte) 11, ram[wch.ptrData]);
            wpsg((byte) 12, ram[wch.ptrData+1]);
        ...
        private void EPM(Ch wch, byte e)
        {
            ...
            //System.err.println($"softPMProcCount[{wch.softPMProcCount:d}]");
            //System.err.println($"softPMStep[{wch.softPMStep:d}]");
            //System.err.println($"softPMStepCount[{wch.softPMStepCount:d}]");
            //System.err.println($"softPMPitch[{wch.softPMPitch:d}]");
            //System.err.println($"KF[{wch.KF:d}]");
            //System.err.println($"NoteNumber[{wch.NoteNumber:d}]");
        ...
        {
            int p = (int)(wch.softPMPitch * 4);
            int hl = (int)(h * 0x100 + l);
            if (hl - p < 0)
            {
                hl = 0;
            }
