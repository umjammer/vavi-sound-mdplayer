...
public class HVL {
    ...
    public static void hvl_reset_some_stuff(Tune ht) {
        for (int i = 0; i < MAX_CHANNELS; i++) {
            Voice v = ht.ht_Voices[i];
            v.vc_Delta = 1;
    ...
    public static boolean hvl_InitSubsong(Tune ht, int nr) {
        ...
        for (int i = 0; i < MAX_CHANNELS; i += 4) {
            ...
            ht.ht_Voices[i + 0].vc_SetPan = ht.ht_defpanleft;
            ...
            ht.ht_Voices[i + 1].vc_SetPan = ht.ht_defpanright;
            ...
            ht.ht_Voices[i + 2].vc_SetPan = ht.ht_defpanright;
            ...
            ht.ht_Voices[i + 3].vc_SetPan = ht.ht_defpanleft;
    ...
    public static void hvl_FreeTune(Tune ht) {
        // No-op in Java (GC handles it)
    }

    public static void hvl_process_stepfx_1(Tune ht, Voice voice, int FX, int FXParam) {
        switch (FX) {
    ...
    public static void hvl_process_stepfx_2(Tune ht, Voice voice, int FX, int FXParam, int[] Note) {
        switch (FX) {
    ...
    public static void hvl_process_stepfx_3(Tune ht, Voice voice, int FX, int FXParam) {
        switch (FX) {
            ...
            case 0x0c: // Volume
                ...
                if (FXParam <= 0x40) {
                    for (int i = 0; i < ht.ht_Channels; i++) {
    ...
    public static void hvl_process_step(Tune ht, Voice voice) {
        ...
        Step step = ht.ht_Tracks[ht.ht_Positions[ht.ht_PosNr].pos_Track[voice.vc_VoiceNum]][ht.ht_NoteNr];
        ...
        if (Instr != 0 && Instr <= ht.ht_InstrumentNr) {
            Instrument Ins = ht.ht_Instruments[Instr];
        ...
        int[] noteArr = {Note};
        hvl_process_stepfx_2(ht, voice, step.stp_FX & 0xf, step.stp_FXParam, noteArr);
        hvl_process_stepfx_2(ht, voice, step.stp_FXb & 0xf, step.stp_FXbParam, noteArr);
        Note = noteArr[0];
    ...
    public static void hvl_plist_command_parse(Tune ht, Voice voice, int FX, int FXParam) {
        switch (FX) {
            case 0:
                if (FXParam > 0 && FXParam < 0x40) {
    ...
    public static void hvl_process_frame(Tune ht, Voice voice) {
        if (voice.vc_TrackOn == 0) {
            return;
        }
        ...
        if (voice.vc_HardCut != 0) {
            ...
            if (nextinst != 0) {
                int d1 = ht.ht_Tempo - voice.vc_HardCut;
                if (d1 < 0) d1 = 0;

                if (voice.vc_NoteCutOn == 0) {
                    voice.vc_NoteCutOn = 1;
                    voice.vc_NoteCutWait = (short) d1;
                    voice.vc_HardCutReleaseF = (short) (-(d1 - ht.ht_Tempo));
                } else {
                    voice.vc_HardCut = 0;
                }
            }
    ...
    public static void hvl_set_audio(Voice voice, double freqf) {
        if (voice.vc_TrackOn == 0) {
        ...
        if (voice.vc_PlantPeriod != 0) {
            voice.vc_PlantPeriod = 0;
            voice.vc_VoicePeriod = voice.vc_AudioPeriod;

            double freq2 = Period2Freq(voice.vc_AudioPeriod);
        ...
        if (voice.vc_NewWaveform != 0) {
            byte[] src = voice.vc_AudioSource_array;
            int srcOff = voice.vc_AudioSource_offset;

            if (voice.vc_Waveform == 4 - 1) {
                System.arraycopy(src, srcOff, voice.vc_VoiceBuffer, 0, 0x280);
            } else {
                int waveLoops = (1 << (5 - voice.vc_WaveLength)) * 5;
                int len = 4 * (1 << voice.vc_WaveLength);
                for (int i = 0; i < waveLoops; i++) {
                    System.arraycopy(src, srcOff, voice.vc_VoiceBuffer, i * len, len);
                }
            }
        ...
        /* Ring Modulation */
        if (voice.vc_RingPlantPeriod != 0) {
            voice.vc_RingPlantPeriod = 0;
            double freq2 = Period2Freq(voice.vc_RingAudioPeriod);
            int delta = (int) (freq2 / freqf);
        ...
        if (voice.vc_RingNewWaveform != 0) {
            byte[] src = voice.vc_RingAudioSource_array;
            int srcOff = voice.vc_RingAudioSource_offset;

            int waveLoops = (1 << (5 - voice.vc_WaveLength)) * 5;
            int len = 4 * (1 << voice.vc_WaveLength);
            for (int i = 0; i < waveLoops; i++) {
                System.arraycopy(src, srcOff, voice.vc_RingVoiceBuffer, i * len, len);
            }
    ...
    public static void hvl_play_irq(Tune ht) {
        if (ht.ht_StepWaitFrames <= 0) {
            if (ht.ht_GetNewPosition != 0) {
                int nextpos = (ht.ht_PosNr + 1 == ht.ht_PositionNr) ? 0 : (ht.ht_PosNr + 1);

                for (int i = 0; i < ht.ht_Channels; i++) {
                    ht.ht_Voices[i].vc_Track = (short) ht.ht_Positions[ht.ht_PosNr].pos_Track[i];
                    ht.ht_Voices[i].vc_Transpose = ht.ht_Positions[ht.ht_PosNr].pos_Transpose[i];
                    ht.ht_Voices[i].vc_NextTrack = (short) ht.ht_Positions[nextpos].pos_Track[i];
        ...
        if (ht.ht_Tempo > 0 && --ht.ht_StepWaitFrames <= 0) {
            ...
            if (ht.ht_PatternBreak != 0) {
                ht.ht_PatternBreak = 0;
                ht.ht_PosNr = (short) ht.ht_PosJump;
                ht.ht_NoteNr = (short) ht.ht_PosJumpNote;
                if (ht.ht_PosNr == ht.ht_PositionNr) {
                    ht.ht_SongEndReached = 1;
                    ht.ht_PosNr = (short) ht.ht_Restart;
                }
                ht.ht_PosJumpNote = 0;
                ht.ht_PosJump = 0;
    ...
    public static void hvl_mixchunk(Tune ht, int samples, short[] buf1, int offset1, short[] buf2, int offset2, int shortStride) {
    ...
    public static void hvl_DecodeFrame(Tune ht, short[] buf1, int offset1, short[] buf2, int offset2, int bufmod) {
        int samples = (int) (ht.ht_Frequency / 50 / ht.ht_SpeedMultiplier);
        int loops = ht.ht_SpeedMultiplier;
        int shortStride = bufmod / 2;

        do {
            hvl_play_irq(ht);
            hvl_mixchunk(ht, samples, buf1, offset1, buf2, offset2, shortStride);
