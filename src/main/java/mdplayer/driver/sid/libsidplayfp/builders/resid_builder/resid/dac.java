...
public class dac
    {
        ...
        public void build_dac_table( short[] dac, int bits, double _2R_div_R, Boolean term)
        {
            ...
            for (int set_bit = 0; set_bit < bits; set_bit++)
            {
                ...
                double Rn = term ?        // Rn = 2R for correct termination,
                  _2R : Float.POSITIVE_INFINITY;// INFINITY;         // INFINITY for missing termination.

                // Calculate DAC "tail" resistance by repeated parallel substitution.
                for (bit = 0; bit < set_bit; bit++)
                {
                    if (Rn == Float.POSITIVE_INFINITY)// INFINITY)
                    {
                        Rn = R + _2R;
                    }
                    else
