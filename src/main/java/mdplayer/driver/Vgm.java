...
logger.log(Level.WARNING, "[%s]:unknown command: adr: 0x%x cmd: 0x%x".formatted(model, vgmAdr, vgmBuf[vgmAdr]));
            ...
            if (countNum > 100) {
                if (model == mdplayer.Common.EnmModel.RealModel && countNum % 100 == 0) {
                    isDataBlock = true;
                    plugin.audio.chipRegister.chip(Ym2608Chip.class).sendDataYM2608(0, model);
                    plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608SyncWait(0, 1);
                    plugin.audio.chipRegister.chip(Ym2151Chip.class).sendDataYM2151(0, model);
                    plugin.audio.chipRegister.chip(Ym2151Chip.class).setYM2151SyncWait(0, 1);

                    plugin.audio.chipRegister.chip(Ym2608Chip.class).sendDataYM2608(1, model);
                    plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608SyncWait(1, 1);
                    plugin.audio.chipRegister.chip(Ym2151Chip.class).sendDataYM2151(1, model);
                    plugin.audio.chipRegister.chip(Ym2151Chip.class).setYM2151SyncWait(1, 1);
                }
            }
        }
        ...
        if (model == mdplayer.Common.EnmModel.RealModel) {
            if (vgmSpeed == 1) { // Apply weight only when speed is constant
                if (useChipYM2612Ch6)
                    plugin.audio.chipRegister.chip(Ym2612Chip.class).setYM2612SyncWait(0, vgmWait);
...
//           oneFrameVGMStream();
    ...
    private void vcGGPSGPort06() {
        plugin.audio.chipRegister.chip(Sn76489Chip.class).setSN76489RegisterGGpanning(vgmBuf[vgmAdr] == 0x4f ? 0 : 1, vgmBuf[vgmAdr + 1] & 0xff, model);
        vgmAdr += 2;
    }

    private void vcPSG() {
        plugin.audio.chipRegister.chip(Sn76489Chip.class).setSN76489Register(vgmBuf[vgmAdr] == 0x50 ? 0 : 1, vgmBuf[vgmAdr + 1] & 0xff, model);
        vgmAdr += 2;
    }

    private void vcAY8910() {
        plugin.audio.chipRegister.chip(Ay8910Chip.class).setAY8910Register((vgmBuf[vgmAdr + 1] & 0x80) == 0 ? 0 : 1, vgmBuf[vgmAdr + 1] & 0x7f, vgmBuf[vgmAdr + 2] & 0xff, model);
        //plugin.audio.chipRegister.setAY8910Register(0, vgmBuf[vgmAdr + 1] & 0xff, vgmBuf[vgmAdr + 2] & 0xff, model);
        vgmAdr += 3;
    }

    private void vcDMG() {
        plugin.audio.chipRegister.chip(DmgChip.class).setDMGRegister((vgmBuf[vgmAdr + 1] & 0x80) == 0 ? 0 : 1, vgmBuf[vgmAdr + 1] & 0x7f, vgmBuf[vgmAdr + 2] & 0xff, model);
        //plugin.audio.chipRegister.setAY8910Register(0, vgmBuf[vgmAdr + 1] & 0xff, vgmBuf[vgmAdr + 2] & 0xff, model);
        vgmAdr += 3;
    }

    private void vcNES() {
        plugin.audio.chipRegister.chip(NesChip.class).setNESRegister((vgmBuf[vgmAdr + 1] & 0x80) == 0 ? 0 : 1, vgmBuf[vgmAdr + 1] & 0x7f, vgmBuf[vgmAdr + 2] & 0xff, model);
        vgmAdr += 3;
    }

    private void vcMultiPCM() {
        plugin.audio.chipRegister.chip(MultiPcmChip.class).setMultiPCMRegister((vgmBuf[vgmAdr + 1] & 0x80) == 0 ? 0 : 1, vgmBuf[vgmAdr + 1] & 0x7f, vgmBuf[vgmAdr + 2] & 0xff, model);
        vgmAdr += 3;
    }

    private void vcMultiPCMSetBank() {
        plugin.audio.chipRegister.chip(MultiPcmChip.class).setMultiPCMSetBank((vgmBuf[vgmAdr + 1] & 0x80) == 0 ? 0 : 1, vgmBuf[vgmAdr + 1] & 0x7f, (vgmBuf[vgmAdr + 2] & 0xff) + (vgmBuf[vgmAdr + 3] & 0xff) * 0x100, model);
        vgmAdr += 4;
    }

    private void vcQSound() {
        plugin.audio.chipRegister.chip(QSoundChip.class).setQSoundRegister(0, vgmBuf[vgmAdr + 1] & 0xff, vgmBuf[vgmAdr + 2] & 0xff, vgmBuf[vgmAdr + 3] & 0xff, model);
        vgmAdr += 4;
    }

    private void vcX1_010() {
        plugin.audio.chipRegister.chip(X1_010Chip.class).setX1_010Register((vgmBuf[vgmAdr + 1] & 0x80) == 0 ? 0 : 1, vgmBuf[vgmAdr + 1] & 0x7f, vgmBuf[vgmAdr + 2] & 0xff, vgmBuf[vgmAdr + 3] & 0xff, model);
        vgmAdr += 4;
    }

    private void vcYM2413() {
        plugin.audio.chipRegister.chip(Ym2413Chip.class).setYM2413Register((vgmBuf[vgmAdr] & 0x80) == 0 ? 0 : 1, vgmBuf[vgmAdr + 1] & 0xff, vgmBuf[vgmAdr + 2] & 0xff, model);
        vgmAdr += 3;
    }

    private void vcYM3812() {
        plugin.audio.chipRegister.chip(Ym3812Chip.class).setYM3812Register((vgmBuf[vgmAdr] & 0x80) == 0 ? 0 : 1, vgmBuf[vgmAdr + 1] & 0xff, vgmBuf[vgmAdr + 2] & 0xff, model);
        vgmAdr += 3;
    }

    private void vcHuC6280() {
        plugin.audio.chipRegister.chip(HuC6280Chip.class).setHuC6280Register((vgmBuf[vgmAdr + 1] & 0x80) == 0 ? 0 : 1, vgmBuf[vgmAdr + 1] & 0x7f, vgmBuf[vgmAdr + 2] & 0xff, model);
        vgmAdr += 3;
    }

    private void vcGA20() {
        plugin.audio.chipRegister.chip(Ga20Chip.class).setGA20Register((vgmBuf[vgmAdr + 1] & 0x80) == 0 ? 0 : 1, vgmBuf[vgmAdr + 1] & 0x7f, vgmBuf[vgmAdr + 2] & 0xff, model);
        vgmAdr += 3;
    }

    private void vcYM2612Port0() {
        plugin.audio.chipRegister.chip(Ym2612Chip.class).setYM2612Register((vgmBuf[vgmAdr] & 0x80) == 0 ? 0 : 1, 0, vgmBuf[vgmAdr + 1] & 0xff, vgmBuf[vgmAdr + 2] & 0xff, model, vgmFrameCounter);
        vgmAdr += 3;
    }

    private void vcYM2612Port1() {
        plugin.audio.chipRegister.chip(Ym2612Chip.class).setYM2612Register((vgmBuf[vgmAdr] & 0x80) == 0 ? 0 : 1, 1, vgmBuf[vgmAdr + 1] & 0xff, vgmBuf[vgmAdr + 2] & 0xff, model, vgmFrameCounter);
        vgmAdr += 3;
    }

    private void vcYM2203() {
        plugin.audio.chipRegister.chip(Ym2203Chip.class).setYM2203Register((vgmBuf[vgmAdr] & 0x80) == 0 ? 0 : 1, vgmBuf[vgmAdr + 1] & 0xff, vgmBuf[vgmAdr + 2] & 0xff, model);
        vgmAdr += 3;
    }

    private void vcYM2608Port0() {
        plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608Register((vgmBuf[vgmAdr] & 0x80) == 0 ? 0 : 1, 0, vgmBuf[vgmAdr + 1] & 0xff, vgmBuf[vgmAdr + 2] & 0xff, model);
        vgmAdr += 3;
    }
...
//            //dat &= 0xdf;
//        }
        plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608Register((vgmBuf[vgmAdr] & 0x80) == 0 ? 0 : 1, 1, adr, dat, model);
        vgmAdr += 3;
    }

    private void vcYM2610Port0() {
        plugin.audio.chipRegister.chip(Ym2610Chip.class).setYM2610Register((vgmBuf[vgmAdr] & 0x80) == 0 ? 0 : 1, 0, vgmBuf[vgmAdr + 1] & 0xff, vgmBuf[vgmAdr + 2] & 0xff, model);
        vgmAdr += 3;
    }
    ...
    private void vcYM2610Port1() {
        int adr = vgmBuf[vgmAdr + 1] & 0xff;
        int dat = vgmBuf[vgmAdr + 2] & 0xff;
        plugin.audio.chipRegister.chip(Ym2610Chip.class).setYM2610Register((vgmBuf[vgmAdr] & 0x80) == 0 ? 0 : 1, 1, adr, dat, model);
        vgmAdr += 3;
    }

    private void vcYMF262Port0() {
        plugin.audio.chipRegister.chip(YmF262Chip.class).setYMF262Register((vgmBuf[vgmAdr] & 0x80) == 0 ? 0 : 1, 0, vgmBuf[vgmAdr + 1] & 0xff, vgmBuf[vgmAdr + 2] & 0xff, model);
        vgmAdr += 3;
    }
    ...
    private void vcYMF262Port1() {
        int adr = vgmBuf[vgmAdr + 1] & 0xff;
        int dat = vgmBuf[vgmAdr + 2] & 0xff;
        plugin.audio.chipRegister.chip(YmF262Chip.class).setYMF262Register((vgmBuf[vgmAdr] & 0x80) == 0 ? 0 : 1, 1, adr, dat, model);
        vgmAdr += 3;
    }

    private void vcYM3526() {
        plugin.audio.chipRegister.chip(Ym3526Chip.class).setYM3526Register((vgmBuf[vgmAdr] & 0x80) == 0 ? 0 : 1, vgmBuf[vgmAdr + 1] & 0xff, vgmBuf[vgmAdr + 2] & 0xff, model);
        vgmAdr += 3;
    }

    private void vcY8950() {
        plugin.audio.chipRegister.chip(Y8950Chip.class).setY8950Register((vgmBuf[vgmAdr] & 0x80) == 0 ? 0 : 1, vgmBuf[vgmAdr + 1] & 0xff, vgmBuf[vgmAdr + 2] & 0xff, model);
        vgmAdr += 3;
    }

    private void vcYMZ280B() {
        plugin.audio.chipRegister.chip(YmZ280BChip.class).setYMZ280BRegister((vgmBuf[vgmAdr] & 0x80) == 0 ? 0 : 1, vgmBuf[vgmAdr + 1] & 0xff, vgmBuf[vgmAdr + 2] & 0xff, model);
        vgmAdr += 3;
    }

    private void vcYMF271() {
        plugin.audio.chipRegister.chip(YmF271Chip.class).setYMF271Register(
    ...
    private void vcYMF278B() {
        plugin.audio.chipRegister.chip(YmF278BChip.class).setYMF278BRegister(
                (vgmBuf[vgmAdr + 1] & 0x80) == 0 ? 0 : 1
                , vgmBuf[vgmAdr + 1] & 0x7f
                , vgmBuf[vgmAdr + 2] & 0xff
                , vgmBuf[vgmAdr + 3] & 0xff
...
//logger.log(Level.TRACE, "fm:%02x:%02x:%02x:".formatted(vgmBuf[vgmAdr + 1] & 0x7f, vgmBuf[vgmAdr + 2], vgmBuf[vgmAdr + 3]));
    ...
    private void vcYM2151() {
        plugin.audio.chipRegister.chip(Ym2151Chip.class).setYM2151Register((vgmBuf[vgmAdr] & 0x80) == 0 ? 0 : 1, 0, vgmBuf[vgmAdr + 1] & 0xff, vgmBuf[vgmAdr + 2] & 0xff, model, (vgmBuf[vgmAdr] & 0x80) == 0 ? ym2151Hosei[0] : ym2151Hosei[1], vgmFrameCounter);
        vgmAdr += 3;
    }

    private void vcOKIM6258() {
        plugin.audio.chipRegister.chip(OkiM6258Chip.class).writeOKIM6258(0, vgmBuf[vgmAdr + 0x01] & 0x7f, vgmBuf[vgmAdr + 0x02] & 0xff, model);
        vgmAdr += 3;
    }

    private void vcOKIM6295() {
        plugin.audio.chipRegister.chip(OkiM6295Chip.class).writeOKIM6295((vgmBuf[vgmAdr + 0x01] & 0x80) == 0 ? 0 : 1, vgmBuf[vgmAdr + 0x01] & 0x7f, vgmBuf[vgmAdr + 0x02] & 0xff, model);
        vgmAdr += 3;
    }

    private void vcSAA1099() {
        plugin.audio.chipRegister.chip(Saa1099Chip.class).writeSAA1099((vgmBuf[vgmAdr + 1] & 0x80) == 0 ? 0 : 1, vgmBuf[vgmAdr + 1] & 0x7f, vgmBuf[vgmAdr + 2] & 0xff, model);
        vgmAdr += 3;
    }

    private void vcWSwan() {
        plugin.audio.chipRegister.chip(WSwanChip.class).writeWSwan((vgmBuf[vgmAdr + 1] & 0x80) == 0 ? 0 : 1, vgmBuf[vgmAdr + 1] & 0x7f, vgmBuf[vgmAdr + 2] & 0xff, model);
        vgmAdr += 3;
    }

    private void vcWSwanMem() {
        plugin.audio.chipRegister.chip(WSwanChip.class).writeWSwanMem(0, (vgmBuf[vgmAdr + 0x01] & 0xff) | ((vgmBuf[vgmAdr + 0x02] & 0xff) << 8), vgmBuf[vgmAdr + 0x03] & 0xff, model);
        vgmAdr += 4;
    }

    private void vcPOKEY() {
        plugin.audio.chipRegister.chip(PokeyChip.class).writePOKEY((vgmBuf[vgmAdr + 1] & 0x80) == 0 ? 0 : 1, vgmBuf[vgmAdr + 1] & 0x7f, vgmBuf[vgmAdr + 2] & 0xff, model);
        vgmAdr += 3;
    }

    private void vcSEGAPCM() {
//logger.log(Level.TRACE, "%4X %4X".formatted(vgmBuf[vgmAdr + 0x01], vgmBuf[vgmAdr + 0x02]));
        plugin.audio.chipRegister.chip(SegaPcmChip.class).writeSEGAPCM(0, (vgmBuf[vgmAdr + 0x01] & 0xff) | ((vgmBuf[vgmAdr + 0x02] & 0xff) << 8), vgmBuf[vgmAdr + 0x03] & 0xff, model);
        vgmAdr += 4;
    }
    ...
    private void vcDataBlock() {
        ...
        case 0x80:
            ...
            case 0x80:
                 // SEGA PCM
                plugin.audio.chipRegister.chip(SegaPcmChip.class).writeSEGAPCMPCMData(chipId, romSize, startAddress, bLen - 8, vgmBuf, vgmAdr + 15, model);
            ...
            case 0x81:
                ...
                plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608Register(chipId, 0x1, 0x00, 0x20, model);
                plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608Register(chipId, 0x1, 0x00, 0x21, model);
                plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608Register(chipId, 0x1, 0x00, 0x00, model);

                plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608Register(chipId, 0x1, 0x10, 0x00, model);
                plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608Register(chipId, 0x1, 0x10, 0x80, model);

                plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608Register(chipId, 0x1, 0x00, 0x61, model);
                plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608Register(chipId, 0x1, 0x00, 0x68, model);
                plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608Register(chipId, 0x1, 0x01, opnaRamType, model);

                if (opnaRamType != 2) {
                    plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608Register(chipId, 0x1, 0x02, (startAddress >> 2) & 0xff, model);
                    plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608Register(chipId, 0x1, 0x03, (startAddress >> 10) & 0xff, model);
                } else {
                    plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608Register(chipId, 0x1, 0x02, (startAddress >> 5) & 0xff, model);
                    plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608Register(chipId, 0x1, 0x03, (startAddress >> 13) & 0xff, model);
                }
                plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608Register(chipId, 0x1, 0x04, 0xff, model);
                plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608Register(chipId, 0x1, 0x05, 0xff, model);
                plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608Register(chipId, 0x1, 0x0c, 0xff, model);
                plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608Register(chipId, 0x1, 0x0d, 0xff, model);

                // Data Transfer
                for (int cnt = 0; cnt < bLen - 8; cnt++) {
                    plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608Register(chipId, 0x1, 0x08, vgmBuf[vgmAdr + 15 + cnt] & 0xff, model);
                }
                plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608Register(chipId, 0x1, 0x00, 0x00, model);
                plugin.audio.chipRegister.chip(Ym2608Chip.class).setYM2608Register(chipId, 0x1, 0x10, 0x80, model);
...
//                plugin.audio.chipRegister.getChip(Ym2608Chip.class).setYM2608Register(0x1, 0x10, 0x80, model);
                ...
                while ((plugin.audio.chipRegister.chip(Ym2608Chip.class).getYM2608Register(chipId, 0x1, 0x00, model) & 0xbf) != 0) {
                ...
                plugin.audio.chipRegister.chip(Ym2608Chip.class).sendDataYM2608(chipId, model);
            ...
            case 0x82:
                ...
                if (ym2610AdpcmA[chipId].length > 0) {
                    ...
                    if (model == mdplayer.Common.EnmModel.VirtualModel)
                        plugin.audio.chipRegister.chip(Ym2610Chip.class).writeYm2610_SetAdpcmA(chipId, ym2610AdpcmA[chipId], model);
                    else
                        plugin.audio.chipRegister.chip(Ym2610Chip.class).writeYm2610_SetAdpcmA(chipId, model, startAddress, bLen - 8, vgmBuf, vgmAdr + 15);
            ...
            case 0x83:
                ...
                if (ym2610AdpcmB[chipId].length > 0) {
                    ...
                    if (model == mdplayer.Common.EnmModel.VirtualModel)
                        plugin.audio.chipRegister.chip(Ym2610Chip.class).WriteYM2610_SetAdpcmB(chipId, ym2610AdpcmB[chipId], model);
                    else
                        plugin.audio.chipRegister.chip(Ym2610Chip.class).WriteYM2610_SetAdpcmB(chipId, model, startAddress, bLen - 8, vgmBuf, vgmAdr + 15);
            ...
            case 0x84:
                // YMF278B
                plugin.audio.chipRegister.chip(YmF278BChip.class).writeYmF278BPCMData(chipId, romSize, startAddress, bLen - 8, vgmBuf, vgmAdr + 15, model);
                dumpData(model, "YMF278B_PCMData", vgmAdr + 15, bLen - 8);
                break;

            case 0x85:
                // YMF271
                plugin.audio.chipRegister.chip(YmF271Chip.class).writeYmF271PCMData(chipId, romSize, startAddress, bLen - 8, vgmBuf, vgmAdr + 15, model);
                dumpData(model, "YMF271_PCMData", vgmAdr + 15, bLen - 8);
                break;

            case 0x86:
                // YMZ280B
                plugin.audio.chipRegister.chip(YmZ280BChip.class).writeYmZ280BPCMData(chipId, romSize, startAddress, bLen - 8, vgmBuf, vgmAdr + 15, model);
                dumpData(model, "YMZ280B_PCMData", vgmAdr + 15, bLen - 8);
                break;

            case 0x87:
                // YMF278B
                plugin.audio.chipRegister.chip(YmF278BChip.class).writeYmF278BPCMRAMData(chipId, romSize, startAddress, bLen - 8, vgmBuf, vgmAdr + 15, model);
                dumpData(model, "YMF278B_PCMRAMData", vgmAdr + 15, bLen - 8);
                break;

            case 0x88:
                // Y8950
                plugin.audio.chipRegister.chip(Y8950Chip.class).writeY8950PCMData(chipId, romSize, startAddress, bLen - 8, vgmBuf, vgmAdr + 15, model);
                dumpData(model, "Y8950_PCMData", vgmAdr + 15, bLen - 8);
                break;

            case 0x89:
                // MultiPCM
                plugin.audio.chipRegister.chip(MultiPcmChip.class).writeMultiPCMPCMData(chipId, romSize, startAddress, bLen - 8, vgmBuf, vgmAdr + 15, model);
                dumpData(model, "MultiPCM_PCMData", vgmAdr + 15, bLen - 8);
                break;

            case 0x8b:
                // OKIM6295
                plugin.audio.chipRegister.chip(OkiM6295Chip.class).writeOKIM6295PCMData(chipId, romSize, startAddress, bLen - 8, vgmBuf, vgmAdr + 15, model);
                dumpData(model, "OKIM6295_PCMData", vgmAdr + 15, bLen - 8);
                break;

            case 0x8c:
                // K054539
                plugin.audio.chipRegister.chip(K054539Chip.class).writeK054539PCMData(chipId, romSize, startAddress, bLen - 8, vgmBuf, vgmAdr + 15, model);
                dumpData(model, "K054539_PCMData", vgmAdr + 15, bLen - 8);
                break;

            case 0x8d:
                // C140
                plugin.audio.chipRegister.chip(C140Chip.class).writeC140PCMData(chipId, romSize, startAddress, bLen - 8, vgmBuf, vgmAdr + 15, model);
                dumpData(model, "C140_PCMData", vgmAdr + 15, bLen - 8);
                break;

            case 0x8e:
                // K053260
                plugin.audio.chipRegister.chip(K053260Chip.class).writeK053260PCMData(chipId, romSize, startAddress, bLen - 8, vgmBuf, vgmAdr + 15, model);
                dumpData(model, "K053260_PCMData", vgmAdr + 15, bLen - 8);
                break;

            case 0x8f:
                // QSound
                plugin.audio.chipRegister.chip(QSoundChip.class).writeQSoundPCMData(chipId, romSize, startAddress, bLen - 8, vgmBuf, vgmAdr + 15, model);
                dumpData(model, "QSound_PCMData", vgmAdr + 15, bLen - 8);
                break;

            case 0x91:
                // X1-010
                plugin.audio.chipRegister.chip(X1_010Chip.class).writeX1_010PCMData(chipId, romSize, startAddress, bLen - 8, vgmBuf, vgmAdr + 15, model);
                dumpData(model, "X1-010_PCMData", vgmAdr + 15, bLen - 8);
                break;

            case 0x92:
                // C352
                plugin.audio.chipRegister.chip(C352Chip.class).writeC352PCMData(chipId, romSize, startAddress, bLen - 8, vgmBuf, vgmAdr + 15, model);
                dumpData(model, "C352_PCMData", vgmAdr + 15, bLen - 8);
                break;

            case 0x93:
                // GA20
                plugin.audio.chipRegister.chip(Ga20Chip.class).writeGA20PCMData(chipId, romSize, startAddress, bLen - 8, vgmBuf, vgmAdr + 15, model);
        ...
        case 0xc0:
            ...
            try {
                switch (bType & 0xff) {
                case 0xc0:
                    plugin.audio.chipRegister.chip(Rf5C68Chip.class).writeRF5C68PCMData(chipId, stAdr, dataSize, vgmBuf, vgmAdr + 9, model);
                    dumpData(model, "RF5C68_PCMData(8BitMonoSigned)", vgmAdr + 9, dataSize);
                    break;
                case 0xc1:
                    plugin.audio.chipRegister.chip(Rf5C164Chip.class).writeRF5C164PCMData(chipId, stAdr, dataSize, vgmBuf, vgmAdr + 9, model);
                    dumpData(model, "RF5C164_PCMData(8BitMonoSigned)", vgmAdr + 9, dataSize);
                    break;
                case 0xc2:
                    plugin.audio.chipRegister.chip(NesChip.class).writeNESPCMData(chipId, stAdr, dataSize, vgmBuf, vgmAdr + 9, model);
    ...
    private void vcPCMRamWrite() {
        ...
        if (pcmAdr != null) {
            if (bType == 0x01) {
                plugin.audio.chipRegister.chip(Rf5C68Chip.class).writeRF5C68PCMData(0, bWriteOffset, bSize, pcmBank[bType].data, pcmAdr, model);
            }
            if (bType == 0x02) {
                plugin.audio.chipRegister.chip(Rf5C164Chip.class).writeRF5C164PCMData(0, bWriteOffset, bSize, pcmBank[bType].data, pcmAdr, model);
            }
        }
...
//logger.log(Level.DEBUG, vgmAdr + ": " + (vgmBuf[vgmAdr] & 0xff) + ", " + ((vgmBuf[vgmAdr] & 0xff) - 0x6f));
    ...
    private void vcWaitNSamplesAndSendYM26120x2a() {
        ...
        plugin.audio.chipRegister.chip(Ym2612Chip.class).setYM2612Register(0, 0, 0x2a, dat, model, vgmFrameCounter);
...
//        }
    ...
    private void vcRf5c68() {
        ...
        plugin.audio.chipRegister.chip(Rf5C68Chip.class).writeRF5C68(id, cmd, vgmBuf[vgmAdr + 2] & 0xff, model);
        vgmAdr += 3;
    }

    private void vcRf5c68MemoryWrite() {
        int offset = ByteUtil.readLeShort(vgmBuf, vgmAdr + 1);
        plugin.audio.chipRegister.chip(Rf5C68Chip.class).writeRF5C68MemW(0, offset, vgmBuf[vgmAdr + 3] & 0xff, model);
        vgmAdr += 4;
    }
    ...
    private void vcRf5c164() {
        int id = (vgmBuf[vgmAdr + 1] & 0x80) != 0 ? 1 : 0;
        int cmd = vgmBuf[vgmAdr + 1] & 0x7f;
        plugin.audio.chipRegister.chip(Rf5C164Chip.class).writeRF5C164(id, cmd, vgmBuf[vgmAdr + 2] & 0xff, model);
        vgmAdr += 3;
    }

    private void vcRf5c164MemoryWrite() {
        int offset = ByteUtil.readLeShort(vgmBuf, vgmAdr + 1);
        plugin.audio.chipRegister.chip(Rf5C164Chip.class).writeRF5C164MemW(0, offset, vgmBuf[vgmAdr + 3] & 0xff, model);
        vgmAdr += 4;
    }
    ...
    private void vcPWM() {
        ...
        plugin.audio.chipRegister.chip(PwmChip.class).writePWM(0, cmd, data, model);
        vgmAdr += 3;
    }
    ...
    private void vcK051649() {
        ...
        vgmAdr += 4;
        plugin.audio.chipRegister.chip(K051649Chip.class).writeK051649(scc1_chipId, (scc1_port << 1) | 0x00, scc1_offset, model);
        plugin.audio.chipRegister.chip(K051649Chip.class).writeK051649(scc1_chipId, (scc1_port << 1) | 0x01, rDat, model);
    ...
    private void vcK053260() {
        ...
        plugin.audio.chipRegister.chip(K053260Chip.class).writeK053260(id, adr, data, model);
        vgmAdr += 3;
    }
    ...
    private void vcK054539() {
        ...
        plugin.audio.chipRegister.chip(K054539Chip.class).writeK054539(id, adr, data, model);
        vgmAdr += 4;
    }
    ...
    private void vcC140() {
        ...
        int adr = (vgmBuf[vgmAdr + 1] & 0x7f) * 0x100 + (vgmBuf[vgmAdr + 2] & 0xff);
        int data = vgmBuf[vgmAdr + 3] & 0xff;
        plugin.audio.chipRegister.chip(C140Chip.class).writeC140(id, adr, data, model);
        vgmAdr += 4;
    }
    ...
    private void vcC352() {
        ...
        plugin.audio.chipRegister.chip(C352Chip.class).writeC352(id, adr, data, model);
        vgmAdr += 5;
    }
