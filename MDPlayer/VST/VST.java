    ...
    public class VST : IDisposable
    {
        ...
        public int hostcallback(IntPtr effect, int opcode, int index, int value, IntPtr ptr, float opt)
        {
            ...
            if (opcode == 3)//audioMasterIdle)
            {
                ...
                {
                    System.err.println("(Future idle calls will not be displayed!)");
                    wasIdle = true;
                }
            }

            //if (!filtered)
                //System.err.println(string.Format("PLUG> HostCallback (opcode {0})\n index = {1}, value = {2}, ptr = {3}, opt = {4}\n", opcode, index, value, ptr, opt));
        ...
        public boolean LoadVST(String filename)
        {
        ...
        public boolean OpenEditor()
        {
            if ((effect.flags & 1) == 0)
            {
                System.err.println("This plug does not have an editor!\n");
                return false;
            }
        ...
        public void processReplacing(int sampleFrames)
        {
            //System.err.println("HOST> Process Replacing...\n");
        ...
        private extern static boolean FreeLibrary(IntPtr hModule);
